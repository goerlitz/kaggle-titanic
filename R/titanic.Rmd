...
### Numeric feature combinations and their influence on survival

```{r, echo=FALSE, warning=FALSE}
# convert to factors
train$Survived <- as.factor(train$Survived)

# get numeric features (without 'passengerId')
numericCols <- names(train)[sapply(train, class) %in% c("numeric")]
integerCols <- names(train)[sapply(train, class) %in% c("integer")]
integerCols <- integerCols[!(integerCols %in% c("PassengerId"))]
numericCols <- c(numericCols, integerCols)

# plot numeric feature combinations (with jitter for integer values)
numericDF <- data.frame(train[, numericCols])
for (c in integerCols) numericDF[,c] <- jitter(numericDF[,c])
plot(numericDF, col=train$Survived)
```
...
train <- train[, !(names(train) %in% c('PassengerId', 'Name', 'Ticket'))]
...
g <- roc(Survived ~ prob, data = train)
plot(g)
```
